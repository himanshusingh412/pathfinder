...
→ Do you enjoy picking color themes and designing visual look-and-feel?`;
    } else if (lowerQuery.includes("college") || lowerQuery.toLowerCase().includes("university")) {
...
✓ Complete 1 mock test this week`;
    } else if (lowerQuery.includes("yes") || lowerQuery.includes("tried") || lowerQuery.includes("done")) {
      reply = `🎯 Next Level Steps

• Awesome!
→ Hands-on practice is the fastest way to learn.

• Action Step:
→ Start building simple projects (like a personal portfolio website).
→ Push your code to GitHub to track progress.

• Question:
→ What project or topic did you work on?`;
    } else if (lowerQuery.includes("first time") || lowerQuery.includes("no") || lowerQuery.includes("never")) {
      reply = `🎯 Getting Started

• Don't worry!
→ Every great engineer started from scratch.
→ You don't need prior experience to start learning.

• Action Step:
→ Check out free YouTube channels like code.org or freeCodeCamp.

• Question:
→ Would you like to explore technology first, or start with business and design?`;
    } else {
